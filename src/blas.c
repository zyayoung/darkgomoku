...
{
    int i;
    for(i = 0; i < N; ++i) X[i*INCX] = pow(X[i*INCX], POW);
}
...
void logsoftmax(float *input, int n, float temp, float *output, int stride)
{
    int i;
    float sum = 0;
    float largest = -FLT_MAX;
    ...
    for(i = 0; i < n; ++i){
        float e = exp(log(input[i*stride] + 1e-6)/temp - log(largest + 1e-6)/temp);
        sum += e;
        output[i*stride] = e;
    }
    for(i = 0; i < n; ++i){
